# Leaf 019 Translation

## Source Pack

- Issue ID: `wholeearthcatalo00unse_1`
- Access leaf: `n19`; canonical physical scan leaf: `20` (`wholeearthcatalo00unse_1_0020.djvu`).
- Printed page: `18`; page type: `Normal`; hand side: `LEFT`.
- Scan URL: https://archive.org/download/wholeearthcatalo00unse_1/page/n19_w500.jpg
- High-resolution scan URL: https://archive.org/download/wholeearthcatalo00unse_1/page/n19_w2000.jpg
- OCR source: official Internet Archive DjVu XML at `_local/page_xml/wholeearthcatalo00unse_1_djvu.xml`; 1739 OCR words / 209 OCR lines. No supplemental OCR used.
- Scan verification: required before orchestrator acceptance.

### Official OCR Line Transcript

The following is source evidence only. It preserves official OCR line order
and errors; it is not an approved reading order or a translation.

~~~text
Shelte
Edited by Lloyd Kahn
S
A book (the first) for domebuilders, with detailed step-by-
step instructions on building 7 different type domes, most
of them geodesic.
A portion of the book describes domes built as part of the
curriculum at an experimental high school, several of the
_structures built almost entirely by 15-17 year olds.
There are both simple and detailed explanations of geodesic
geometry, exterior photos of the different domes, interior
fisheye photos, and sketches of details.
Geodesic chord factors (constants that allow you to cal-—
culate strut lengths for different size domes) are published
here for the first time——up to 6 frequency for “alternate
breakdown,” to 8 frequency for “triacon breakdown.”
The nature of the dome designs is experimental; the pur-
pose of the book is to communicate the builders’ exper-
ience; the hope of the writers is to initiate individual
dome building, innovations, & prototypes.
[The above is Lloyd trying to review his own book. He’s
hopelessly modest. If you’re doming, the book is simply
necessary. —SB]
*
Domebook One
Plywood Domes
We have built seven domes like this in about three months’ time. Most
of those on our building crew were 15-17 years old.
We used pipe-section hubs and stainless steel straps for the framework——
a method first used by Fletcher Pence in the Virgin Islands about ten
years ago. The skeleton framework is first strapped into place,
a membrane is then attached, and joints are waterproofed.
We used 2 x 3’s for the skeleton, 3/8’’ Duraply plywood for skin, clear
ultra-violet resistant vinyl in geometric patterns for light, polyurethane
caulk for sealing joints, and other ingredients listed below. The entire
dome——struts and skin——will fit in a 3/4 ton pickup truck.
*% 3 « x x3
Mig xi sot
rand Land Use a : ion a
Domebook One from: °
1970; 56 pp. Pacific Domes
Box 1692
$3.00 postpaid
5 or more $2.25 each
Los Gatos, CA 95030
(except quantity orders)
Tube Fran.e Domes
THE FRAME
Any suitably strong tubing can be used, but the cheapest and easiest
to get is “EMT” electrical conduit. It is easy to work with and is
plated, so painting isn’t necessary. %"’ is not suitable for any domes
that will be subjected to heavy weather conditions, but it is useful
for indoor structures and small (up to 14 feet diameter) domes. 4
conduit will bend if climbed on. %’' is best for most uses. It whole-
or WHOLE EARTH CATALOG
Models are essential. Don’t try to build a dome without first m
and studying models. However, don’t get so involved with mod
that you never trv a real structure. fe
STRUT MODELS
These are models of the structural framework of a dome, made with
1/8" dowels and ‘’D-Stix” rubber connectors. Youcangetthe
connectors from Edmund Scientific Co., 100 Edscorp Building, _
Barrington, N. J. 08007. Dowels can be obtained at a hobby shop |
(they are used by model airplane hobbyists) or you can buy one of
the D-Stix kits from Edmund, with colored dowels.
Making a 3-Frequency Sphere
Your first model should be a sphere, since a dome is a portion of a
sphere. You can then determine where to cut it off, how to orient it
to the earth, and see the relationship of a geodesic sphere to the
icosahedron.
Important note: when figuring the length of model struts, be sure you
subtract for the length of the connector. In the following table, this
has been done.
Here are calculations for a 3-frequency sphere. Check these out yourself d
before cutting, using chord factors below. For ease in multiplying chord
factors, the metric system is preferable. Remember to adjust for connector.
Color Code Make this Many
Strut Chord Factor —_ Length of Strut
A 3486 3 23/32" Red 60
B -4035 4 13/32" Blue 90
Cc A124 4 1/2" Yellow 120
Putting it Together
You are actually making a spherical icosahedron, with its 20 faces subdivided _ .
into smaller triangles. Red struts outline vertices of icosa faces.
Put together one face:
and continue until you have 20 of these subdivided triangles. !t will be
clearer with colors than it looks above.
Lay out 4 x 8 panels on the floor plan of your intended dome in a
pattern that makes sense. Remember if you are using 1 1/8” ply-
wood to keep the tongue and grooves meshing properly. See if
sales for about 9¢/ft. Using the chord factors, you can use %”’ conduit
! in triangles whose sides are up to 4% feet long. 4 feet is maximum
where there will be snow loads. This will result in about a 24 foot
maximum diameter in 3-frequency. For larger domes you will
need bigger tubes or a higher frequency. Bigger tubes are hard to
squash! Think first.
you can do it in a way that the cut off plywood ends can be over-
turned and used upside down at the other side of the dome to fill »
in the empty spaces. Also, if possible, arrange things so awhole oe
4 x 8 will bridge the beam nest over the center pier, as shown. on?
Scrap A is turned over and used at B, locating edge P asshownin
Fig. 1. For a 20’ dome this can mean that you can make the plat-
form from only 11 sheets! (see drawing). Also, space the plywood ~
so that the edges arrive over beams in the strongest manner. If it
all works out well, you will save wood and piers. :
Cutting
The tubes should be cut according to the chord factors plus 1%".
The chord factor gives the ‘‘center-of-hole to center-of-hole”’ —
length, and there must be about %"’ beyond the holes. Conduit
comes in ten foot lengths. You get two struts from each length
for making domes up to about 24 feet diameter.
os
PAWN
scenll SEC
Ae ee eee ean
ENG ANE)
a
VITAL STATISTICS—Pacific Dome
Geometry: 3-frequency geodesic, 5/8 sphere, icosa-alternate breakdown,
vertex zenith
Diameter: 24’
Weight (not including floor) 2050 Ibs
Volume: about 4400 cubic feet
Floor area: (not including lofts) 452 sq ft
Note: volume is a far better measure of living space, especially in a
dome, as you'll not be confined to the floor area.
Squashing
Flatten the tube tips 2%"’ from the ends, by squeezing them in
avise. A big vise. Small ‘home workshop” vises will break.
Kiet
Floor plan
DOME INGREDIENTS
—12 pieces 4’ x 7’ plywood for small triangles
—24 pieces 4’ x 8’ plywood for large triangles
—6 pieces 4’ x 9’ plywood for extra-large triangles
—about 750 lineal feet 2 x 3's for struts (of 8’ and 10’ lengths). Figure
the proper number of each to order.
—61 hubs, cut from sections of pipe
—about 500’ stainless steel strap, about 400 stainless steel buckles
—about 20 Ibs 4d or 6d hot dip galvanized nails
—quantity of window material up to you
—12 tubes of caulk
—2% gallons primer, 2% gallons finish coat paint
—misc. materials for vent, door etc.
—floor materials not included
Tony Magar's hub for soace
ow
Ma
subtract for windows
vet
Insulation is 1 1/8” polystyrene (“styrofoam’’). This is the white — a
you see on the interiors of the plywood domes. You must get os
fire resistant foam, as fires are going to burn very efficiently in
domes. ‘
The foam is pure white, granular, and combined with the wood
struts, gives the dome a half-timbered look. It feels more wooden
than plastic.
Total Cost: $929.00 It’s put in with three small nails per triangle.
Fuller Sun Dome
Blueprints for a 3-frequency geodesic dome. The $5
includes construction license. Designed for swimming
pool covers, dome plans specify wood strips and
cheap polyethylene skin. However, you can modify
to build domes of other materials, such as plywood,
Miscellaneous Ideas
Completely transparent dome with pop-in insulating panels. Adjust
dome to the season, change light patterns to block or admit sunlight,
view different parts of landscape.
Table of struts: “make a large, clear copy of this and post by the saw
Hang a swing in your dome for quick passage from one side to another
Strut Number to cut Length, using 34" Axial Angle Angle at which you . S i : [ 7
2 a ee both ends stad an Aluminum and galvanized steel can be used as skins for domes. Parachutes ADF Cat LOPS AAs NOre SYseauts abe triangles 4%
= thee et pet mop Aluminum doesn’t have to be painted, will last about 25 years make the dome; can be used up to about 35° diameter. E
c 80 55 7/8" 78° 120 (near the ocean it should be anodized), and is soft. Galvanized
“Strut lengths here are based upon
3 1/2” diameter hubs. If you use
different size hubs, calcualte strut
length from chord factors on page
and subtract hub diameter.
[Suggested by Ken Babbs.]
steel has to be painted and is stiff.
You can use thin metal if you spray the inside of the dome with
foam. The panels should be sprayed from the inside with auto-
mobile undercoating or something like it to keep the dome from
booming.
Geodesic Sun Dome
1966
$5.00 postpaid
from:
Sun Dome
Popular Science Monthly
355 Lexington Avenue 6,
New York, N. Y. 10017 ;
With a metal skin and a wood frame the panels can be shingled. Cut
the panels an inch or two larger than the actual size of the triangles
(Then when the panels are attached to the frame they overlap.
See metal tube domes.). For sealing you put a strip of vinyl foam
or tape caulk over the struts before attaching the panels. The panels
can be stapled to the frame. You should use a staple gun that makes
the staples curve outward. For thick metal you can rent an air stapler.
Radial saw
Angle: use an adjustable protractor to double check the saw’s gauge. Hold it against
fence, pull blade out and check to see that blade parallels protractor.
Length: tape measures are made to hook over a piece of wood. For greater accuracy
use the 1” line on the tape measure and line it up with inside of saw blade. Add 1”
to total measurement when setting stop. V-mark stop and table and check the stop
for slipping periodically while sawing.
4 Make sure table and fence are made of clear straight wood. Close one eye and sight
e.
~~~

## Context Notes

- Source pack generated from the verified public-access/DjVu/scandata mapping.
- Identify every visible entry, excerpt, caption, diagram, table, signature,
  and meaningful order/access field against the scan before translation.
- Do not treat OCR line order as page reading order on multi-column layouts.

## Glossary Updates

- geodesic dome：测地穹顶。
- chord factor：弦长系数。
- frequency：分频数。
- strut：杆件。

## Final Translation

# 居所与土地利用

Lloyd Kahn 编辑。

## 《穹顶书一》

这是（第一本）为穹顶建造者写的书，逐步详细说明七种不同穹顶的建造方法，
其中大部分是测地穹顶。

书中一部分介绍某实验高中的课程建造项目；其中几座结构几乎完全由 15–17 岁
学生建成。书里既有测地几何的简明说明，也有详细说明；还有各式穹顶的外观
照片、室内鱼眼照片和细部草图。

测地弦长系数——用来计算不同尺寸穹顶杆件长度的常数——首次在此公布：
“交替细分”最高到 6 分频，“三向细分”最高到 8 分频。

这些穹顶设计本质上属于实验。书的目的是交流建造者的经验；作者希望由此
启动个人的穹顶建造、创新与原型。

〔上面是 Lloyd 试着评论自己的书。他谦虚得没救了。如果你正在做穹顶，
这本书就是必需品。——SB〕

*Domebook One*；1970 年，56 页。邮资已付 3.00 美元；五本以上每本
2.25 美元。购自 Pacific Domes, Box 1692, Los Gatos, CA 95030；或向
WHOLE EARTH CATALOG 订购（批量订单除外）。

### 胶合板穹顶

大约三个月里，我们建了七座这样的穹顶。施工队多数成员 15–17 岁。

框架采用管段轮毂和不锈钢带——Fletcher Pence 约十年前在 Virgin Islands
首次使用这种方法。先把骨架捆扎到位，再附上蒙皮，并对接缝作防水处理。

骨架使用 2×3 英寸木料，蒙皮用 3/8 英寸 Duraply 胶合板，透光处按几何图案
使用透明抗紫外线乙烯基材料，接缝用聚氨酯填缝剂密封，其他材料见下表。
整座穹顶——杆件与蒙皮——可装进一辆 3/4 吨皮卡。

#### Pacific Dome 关键数据

- 几何：3 分频测地结构、5/8 球体、二十面体交替细分、顶点朝天。
- 直径：24 英尺。
- 重量（不含地板）：2,050 磅。
- 体积：约 4,400 立方英尺。
- 地板面积（不含阁楼）：452 平方英尺。
- 注：体积远比面积适合衡量居住空间，尤其穹顶中，人不会受限于地板平面。

#### 穹顶材料

- 12 张 4×7 英尺胶合板，做小三角形。
- 24 张 4×8 英尺胶合板，做大三角形。
- 6 张 4×9 英尺胶合板，做超大三角形。
- 约 750 延尺 2×3 英寸木料作杆件（8 英尺与 10 英尺长度）；应计算各自订购数。
- 61 个由管段切成的轮毂。
- 约 500 英尺不锈钢带，约 400 个不锈钢扣。
- 约 20 磅 4d 或 6d 热浸镀锌钉。
- 窗户材料用量自行决定；计算胶合板时扣除窗户。
- 12 管填缝剂。
- 2.5 加仑底漆，2.5 加仑面漆。
- 通风口、门等杂项材料；不含地板材料。
- 总成本：929.00 美元。

#### 杆件表

“做一份大而清楚的副本，贴在锯子旁边。”以下长度以直径 3.5 英寸的轮毂
为准；若用不同尺寸轮毂，应根据书中弦长系数计算杆长，再减去轮毂直径。

| 杆件 | 切割数量 | 长度 | 两端轴向角 | 径向锯设定角 |
|---|---:|---:|---:|---:|
| A | 30 | 46 11/16 英寸 | 80° | 10° |
| B | 55 | 54 5/8 英寸 | 78° | 12° |
| C | 80 | 55 7/8 英寸 | 78° | 12° |

#### 径向锯

**角度**：用可调量角器复核锯的刻度。把量角器贴住挡板，拉出锯片，检查
锯片是否与量角器平行。

**长度**：卷尺的设计是勾住木料。为提高精度，用卷尺的 1 英寸刻线对齐锯片
内侧；设定止挡时，在总尺寸上加 1 英寸。给止挡与锯台作 V 形记号，锯切时
定期检查止挡是否滑动。

锯台和挡板务必用纹理清楚、笔直的木料制成。闭上一只眼睛沿挡板瞄准。

### 管框穹顶

#### 框架

任何强度合适的管材都可使用，但最便宜、最容易取得的是“EMT”电工导管。
它加工方便且有镀层，不必涂漆。1/2 英寸管不适合承受恶劣天气的穹顶，却可
用于室内结构和小型（直径不超过 14 英尺）穹顶；有人攀爬时它会弯曲。
3/4 英寸管最适合多数用途，批发约每英尺 9 美分。

利用弦长系数，3/4 英寸导管可用于边长不超过 4.5 英尺的三角形；有雪荷载时
最长 4 英尺。采用 3 分频时，这对应约 24 英尺的最大直径。更大的穹顶需要
更粗管材或更高分频。粗管很难压扁！先想清楚。

#### 切割

管材应按弦长系数所得尺寸再加 1.5 英寸来切。弦长系数给出“孔中心到孔中心”
的长度，孔外两端各需留约 3/4 英寸。导管以 10 英尺一根供应；建直径不超过
约 24 英尺的穹顶时，每根可切出两条杆件。

#### 压扁

从管端起 2.25 英寸处把端头放入台钳压平。要用大台钳；小型“家庭工作间”
台钳会断。

### 模型

模型不可缺少。不要在制作、研究模型之前就试着建穹顶；但也别沉迷模型，
以至于永远不尝试真实结构。

#### 杆件模型

这是穹顶结构骨架的模型，用 1/8 英寸圆木棒和“D-Stix”橡胶连接件制作。
连接件购自 Edmund Scientific Co., 100 Edscorp Building, Barrington,
N.J. 08007。圆木棒可在模型店买到（模型飞机爱好者会用），也可从 Edmund
购买带彩色圆木棒的 D-Stix 套件。

#### 制作 3 分频球体

第一个模型应当是球，因为穹顶是球的一部分。之后便可决定在哪里截断、如何
使它相对地面定向，并看出测地球体与二十面体的关系。

重要：计算模型杆长时，务必减去连接件长度。下表已经扣除。切割前请自己用
弦长系数核算。公制更方便相乘；记得针对连接件调整。

| 杆件 | 弦长系数 | 杆长 | 色码 | 数量 |
|---|---:|---:|---|---:|
| A | .3486 | 3 23/32 英寸 | 红 | 60 |
| B | .4035 | 4 13/32 英寸 | 蓝 | 90 |
| C | .4124 | 4 1/2 英寸 | 黄 | 120 |

#### 组装

实际制作的是球形二十面体，它的 20 个面又细分成小三角形。红色杆件勾勒
二十面体各面的顶角。按图先拼一个面，再继续，直到有 20 个这种细分三角面；
实际使用颜色后会比黑白图清楚。

### 地板平面

依照拟建穹顶的地板平面，把 4×8 英尺板材排成合理图案。如果使用 1 1/8 英寸
企口胶合板，要让榫舌与凹槽正确咬合。试着安排，使切掉的胶合板端料可以翻面，
用在穹顶另一侧填空；如果可能，让一整张 4×8 板跨过中央墩上方的梁巢，如图。

A 号边角料翻面用于 B 处，P 边按图 1 定位。对 20 英尺穹顶而言，这能让平台
只用 11 张板！还应安排板材，使边缘以最坚固方式落在梁上。一切顺利，就能
节省木材和墩柱。图中标注：4×8 胶合板、4×6 梁、2×6 搁栅、墩柱、P 边。

连接件示意：Dyna Domes 木杆轮毂；焊在一起的垫圈；Tony Magar 的空间网格
轮毂。

### 保温

保温层采用 1 1/8 英寸聚苯乙烯（“Styrofoam”），也就是胶合板穹顶内侧
看到的白色材料。必须买阻燃泡沫，因为穹顶里的火会烧得非常有效率。

这种泡沫纯白、有颗粒感，与木杆结合后使穹顶有半木构外观；感觉更像木头而非
塑料。每个三角形用三枚小钉固定。

### 杂项构想

- 完全透明的穹顶配可嵌入式保温板。随季节调整穹顶，改变光线图案以遮挡或
  接纳日光，观看景观不同部分。
- 在穹顶里挂秋千，以便迅速从一边摆到另一边。
- 铝和镀锌钢可作穹顶蒙皮。铝无需涂漆，约可用 25 年（海边应阳极氧化），
  而且柔软；镀锌钢必须涂漆，材质较硬。
- 若在穹顶内侧喷泡沫，可使用薄金属；应从内侧给板材喷汽车底盘涂层或类似
  材料，防止穹顶发出轰鸣。
- 金属蒙皮配木框架时，板材可以像瓦片一样叠接。板材比三角形实际尺寸大
  一两英寸，固定到框架时便会重叠。密封时，固定板材前先在杆件上铺一条乙烯基
  泡沫或胶带填缝条。板材可钉在框架上；订钉枪应使钉脚向外弯。厚金属可租
  气动订钉机。

## Fuller 太阳穹顶

这是一套 3 分频测地穹顶蓝图，5 美元中包含建造许可。设计本用于游泳池罩，
方案指定木条和廉价聚乙烯蒙皮；但可以修改，改用胶合板、降落伞或汽车车顶
等材料。一套简单系统：两个三角形组成穹顶，可用于直径约 35 英尺以内。

——Ken Babbs 推荐

*Geodesic Sun Dome*；1966 年。邮资已付 5.00 美元。购自 Sun Dome,
Popular Science Monthly, 355 Lexington Avenue 6, New York, N.Y. 10017。


## Omitted Bibliographic/Order Info

- 无；两套穹顶资料的评论、书目、步骤、材料、数据、表格、图注和地址均已保留。

## OCR / Uncertainty Notes

- 已按高清扫描重建五栏施工顺序。官方 OCR 将地板、模型、管框与材料表交错，
  译文按标题和图示归位；英制尺寸与历史价格依原文，不擅自换算。

## Self Critique

- 已完整翻译杆件模型、管框施工、材料清单、两张尺寸表、地板排料、保温和
  蒙皮建议，书评之外的技术正文没有被删成建造概述。
