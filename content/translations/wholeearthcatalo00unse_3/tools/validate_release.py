#!/usr/bin/env python3
"""Release gate for the January 1971 Chinese translation package."""
...
def validate_issue() -> list[str]:
    ...
    if [row["leaf"] for row in rows] != list(range(EXPECTED_LEAVES)):
        errors.append("status.jsonl must contain contiguous leaves 000-047")
...
def main() -> None:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)
    print("release gate passed: 48 leaves are complete, reviewed, and accepted")
