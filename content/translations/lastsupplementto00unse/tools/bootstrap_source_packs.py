#!/usr/bin/env python3
"""Generate source-pack leaves and initial status for the March 1971 issue."""
...
def page_records() -> list[dict[str, object]]:
    ...
    if len(objects) != 132 or len(access_pages) != 132:
        raise RuntimeError(
            f"Expected 132 OCR objects and 132 access pages; got {len(objects)} and {len(access_pages)}"
        )
    ...
    for leaf, (obj, page) in enumerate(zip(objects, access_pages, strict=True)):
        ...
        section = "Front Matter" if leaf < 2 else "Back Matter" if leaf >= 129 else "Supplement"
